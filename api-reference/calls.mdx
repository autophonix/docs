...
```bash cURL
# List recent calls
curl "https://api.thunderphone.ai/v1/orgs/{org_id}/calls?limit=20" \
  -H "Authorization: Token YOUR_API_TOKEN"

# Filter by agent and direction
curl "https://api.thunderphone.ai/v1/orgs/{org_id}/calls?agent_id=12&direction=inbound" \
  -H "Authorization: Token YOUR_API_TOKEN"
```
...
response = requests.get(
    f"https://api.thunderphone.ai/v1/orgs/{org_id}/calls",
...
const response = await fetch(
  `https://api.thunderphone.ai/v1/orgs/${orgId}/calls?${params}`,
  {
    headers: { 'Authorization': 'Token YOUR_API_TOKEN' }
  }
);
...
```bash
curl https://api.thunderphone.ai/v1/orgs/{org_id}/calls/987654321 \
  -H "Authorization: Token YOUR_API_TOKEN"
```
...
```bash
curl -L https://api.thunderphone.ai/v1/orgs/{org_id}/calls/987654321/audio \
  -H "Authorization: Token YOUR_API_TOKEN" \
  -o recording.mp3
```
...
```bash
curl https://api.thunderphone.ai/v1/orgs/{org_id}/calls/987654321/transcript \
  -H "Authorization: Token YOUR_API_TOKEN"
```
